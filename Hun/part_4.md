...
fork는 부모와 똑같이 자식 프로세스 만드는 것

대부분의 자식은 fork 이후 바로 exec 호출

-> 부모 page 다 복사, 비효율
당분간은 부모 page 공유, 공유중인 page에 쓸 때 copy

vfork는 자식이 부모 주소 공간을 사용 page가 전혀 복사되지 않음

바로 exec 호출되는 것을 가정

### page replacement

demand paging에서는 2가지 문제가 발생한다
1. frame allocation algorithm
2. page-replacement algorithm

이 중 2번재 문제에 대해서 살펴 보자
...
3. LRU
  page마다 마지막 사용 시간 기록
  교체 시 가장 오랫동안 사용되지 않은거 pick
  *SR을 참조열 S의 역이라 정의할 때, SR에서의 algorithm성능은 S에서와 동일하다
  ...
  -> 둘 다 HW 지원 필요하다
4. LRU 근사 알고리즘
  reference bit : 이 page가 참조되었는지 여부
  clock algorithm : 일정 시간마다 참조 비트를 확인
5. paging buffering
  ...

이제 페이징의 첫번째 문제인 frame allocation에 대해 살펴보자

최소로 할당해야 하는 프레임의 수는 컴퓨터 아키텍처에 의해 정의된다

1. equal allocation, 균등 할당
  모든 프로세스에 같은 프레임 할당하기
2. proportional allocation, 비례 할당
  프로세스의 크기 비율에 맞추어 할당하는 방법

두가지 모두 우선순위에 상관없이 동등하게 취급한다는 점에 유의하자

### global vs local allocation, 전역할당 대 지역할당

전역 교체는 프로세스가 교체 할 프레임을 다른 프로세스에 속한 프레임을 포함한 모든 프레임을 대상으로 하는 것

지역 교체는 프로세스가 자신에게 할당된 프레임 중에서만 교체

특정 임계값 아래로 떨어지면 페이지 교체를 시작, 이는 충분한 메모리가 남아 있도록 보장

reaper라는 커널 루틴은 페이지를 회수하기 위한 루틴이다

리눅스에서는 메모리가 부족해지면 OOM킬러(out-of-memory)라는 루틴이 종료할 프로세스를 선택하여 메모리 회수

### non-uniform memory access, 비균등 메모리 접근

지금까지 가정 한 메인 메모리에 동등하게 접근 되는 것은, 여러개의 cpu를 가진 비균등 메모리 접근(NUMA) 시스템에서 이것은 사실이 아니다

NUMA를 고려하면 스케줄러는 프로세스가 마지막으로 실행된 CPU를 추적해야 한다

스케줄러는 각 프로세스를 직전에 실행된 CPU에 스케줄하고, 가상 메모리 시스템은 스케줄 된 CPU와 가까운 프레임을 할당

### thrashing, 쓰레싱

과도한 paging 작업을 스레싱이라고 부른다 어떤 프로세스가 실제 실행 시간보다 더 많은 시간을 페이징에 사용하고 있으면 스레싱이 발생한 것이다

### cause of thrashing, 스레싱의 원인

CPU 이용률이 낮으면 새로운 프로세스를 시스템에 추가해서 다중 프로그래밍의 정도를 높인다

이때 전역 페이지 교체 알고리즘을 사용하여 교체를 수행한다

교체된 페이지가 필요로 하는 페이지였다면 이 역시 페이지 폴트를 발생시킨다

이런 상황이 반복되며 프로세스들이 페이징 장치를 기다리는 동안 CPU 이용률은 또 떨어진다

CPU 스케줄러는 이용률이 떨어지는 것을 보도 다중 프로그래밍의 정도를 더 높인다(악순환)

이를 해결하기 위해서는 다중 프로그래밍의 정도를 낮춰야 한다

지역 교체 알고리즘으로 스레싱의 영향을 제한 할 수 있다

또 스레싱을 방지하기 위해서는 프로세스가 필요로 하는 최소한의 프레임 갯수를 보장해야 한다

이 방법은 프로세스 실행의 지역성 모델을 기반으로 한다

### 작업 집합 모델, working-set model

작업 집합 모델은 지역성을 토대로 하고 있다

### page-fault Frequency, PFF, 페이지 폴트 빈도

PFF 방식은 더 직접적으로 스레싱을 조절

페이지 폴트 비율의 상한과 하한을 정해 놓고 이를 넘어서면 처리해준다

### memory compression, 메모리 압축

페이징의 대안은 메모리 압축

수정된 프레임을 스왑 공간으로 페이징 아웃하지 않고, 여러 프레임을 하나의 프레임으로 압축한다
